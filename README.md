...
- company metadata is now enriched via `python manage.py enrich_company_metadata`, which pulls `description`, `website`, and `exchange` from Yahoo Finance and then applies curated CSV overrides; the implementation plan is documented in [`docs/company-metadata-ingestion-plan.md`](./docs/company-metadata-ingestion-plan.md)
- the first full local metadata backfill filled `499/500` descriptions, `499/500` websites, and `500/500` exchanges; the one remaining gap (`FISV`) now belongs in the override-curation path rather than the ingestion pipeline itself
